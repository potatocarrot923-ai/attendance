...
HTTPS_PORT = 3443
DATA_DIR = Path(os.environ.get("CARROT_DATA_DIR", Path(__file__).parent))
DATA_FILE = DATA_DIR / "carrot_data.json"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in {"/", "/classroom", "/classroom/", "/classroom.html", "/Attendance.html", "/at/Attendance.html"}:
            # Keep the legacy classroom URLs working while serving the actual
            # checked-in frontend file on every public entry point.
            page = Path(__file__).with_name("Attendance.html")
...
if __name__ == "__main__":
    sms_ready = all((SMS_ACCOUNT_SID, SMS_AUTH_TOKEN, SMS_FROM_NUMBER, RECOVERY_PHONE))
    print(f"SMS recovery configuration: {'ready' if sms_ready else 'not configured'}")
    if PUBLIC_MODE:
